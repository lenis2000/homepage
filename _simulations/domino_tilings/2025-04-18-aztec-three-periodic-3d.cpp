...
- 3D height function visualization
...
// Define sleep function that works in both environments
#ifdef __EMSCRIPTEN__
#define SLEEP_YIELD emscripten_sleep(0)
#else
#define SLEEP_YIELD do {} while(0)
#endif
...
vector<Matrix> d3p(const MatrixDouble &x1) {
    ...
    for (int k = 0; k < iterations; k++){
        ...
        AA.push_back(C);
        SLEEP_YIELD; // Yield periodically during heavy computation
    }
    return AA;
}
...
MatrixInt aztecgen(const vector<MatrixDouble> &x0) {
    ...
    for (int i = 0; i < totalIterations; i++){
        ...
        SLEEP_YIELD; // Yield control so that progress updates are visible.
    }
    return a1;
}
...
char* simulateAztec(int n, double w1, double w2, double w3, double w4, double w5, double w6, double w7, double w8, double w9) {
    try {
        ...
        // Create 2*n x 2*n weight matrix with 3x3 periodic weights.
        int dim = 2 * n;
        MatrixDouble A1a(dim, dim, 0.0);
        
        // Store 3x3 weights in a flat array for faster access
        const double W[9] = {w1, w2, w3, w4, w5, w6, w7, w8, w9};

        // Set up the 3x3 periodic weights pattern
        for (int i = 0; i < dim; i++){
            int ii = i % 3;             // row inside the 3-periodic block
            for (int j = 0; j < dim; j++){
                int jj = j % 3;         // column inside the 3-periodic block
                A1a.at(i, j) = W[ii*3 + jj]; // assign the corresponding weight
            }
        }
        ...
        SLEEP_YIELD; // Yield to update UI
            ...
            for (int j = 0; j < size; j++){
                if (dominoConfig.at(i, j) == 1) {
                    ...
                    if (oddI && oddJ) { // i odd, j odd
                        ...
                        color = "#ffff00"; // yellow for left vertical
                        x = j - i - 1;
                        y = size + 1 - (i + j) - 2;
                        w = 2;
                        h = 4;
                    } else if (!oddI && !oddJ) { // i even, j even
                        color = "#0000ff"; // blue for lower horizontal
                        x = j - i - 2;
                        y = size + 1 - (i + j) - 1;
                        w = 4;
                        h = 2;
                    } else if (!oddI && oddJ) { // i even, j odd
                        color = "#ff0000"; // red for right vertical
                        x = j - i - 1;
                        y = size + 1 - (i + j) - 2;
                        w = 2;
                        h = 4;
