/*
  ...
  Usage with Emscripten (example command):
    ...
    emcc 2025-03-28-t-emb-a2-json.cpp -o 2025-03-28-t-emb-a2-json.js \
     -s WASM=1 \
     -s ASYNCIFY=1 \
     ...
     -s SINGLE_FILE=1 \
     -O3 -ffast-math\
     && mv 2025-03-28-t-emb-a2-json.js ../../js/
...
char* doTembJSONwithA(int n, double a) {
    ...
    for (int m = 1; m <= n; m++) {
        ...
        Tarray[m][( m + n)][(0 + n)] = std::complex<double>(+1.0, 0.0);
        Tarray[m][(0 + n)][(-m + n)] = std::complex<double>(0.0, 1.0);
        Tarray[m][(0 + n)][( m + n)] = std::complex<double>(0.0,-1.0);

        // O(-m,0) = +1, O(m,0)= +1, O(0,-m)= i*a, O(0,m)= i*a
        Oarray[m][(-m + n)][(0 + n)] = std::complex<double>(1.0, 0.0);
        Oarray[m][( m + n)][(0 + n)] = std::complex<double>(1.0, 0.0);
        Oarray[m][(0 + n)][(-m + n)] = std::complex<double>(0.0, 1.0);
        Oarray[m][(0 + n)][( m + n)] = std::complex<double>(0.0, 1.0);
    }
