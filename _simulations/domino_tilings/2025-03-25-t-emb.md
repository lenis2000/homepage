...
<div id="controls" style="font-size: 20px;">
  <label for="n-input">Aztec Diamond parameter n (0–300):</label>
  <input id="n-input" type="number" value="10" min="0" max="300" step="1">
  <button id="update-btn">Update</button>
  <button id="cancel-btn" style="display: none; margin-left: 10px; background-color: #f44336; color: white; padding: 5px 10px; border: none; cursor: pointer; border-radius: 4px;">Cancel</button><br>
...
Module.onRuntimeInitialized = async function() {
  ...
  async function updateVisualization(n, progressInterval) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        // Check for cancellation after computation but before rendering
      ...
      // Check for cancellation after building edges
      await checkCancellation();
      ...
      // Check for cancellation before drawing edges
      await checkCancellation();
      ...
      // Check for cancellation after drawing T-vertices
      await checkCancellation();
  ...
  // Remove automatic updates when n changes to prevent freezing with large values
  ...
  function handleUpdate() {
    ...
    if (nVal < 0 || nVal > 300) {
      alert("Please pick integer n in [0, 300].");
      return;
    }
    ...
    // Reset C++ progress counter
    resetProgress();

    // Disable the update button and input field during computation
    const updateBtn = document.getElementById("update-btn");
    const cancelBtn = document.getElementById("cancel-btn");
    const nInput = document.getElementById("n-input");
    ...
    setTimeout(() => {
      // Start computation
      try {
        currentComputation = updateVisualization(nVal, progressInterval)
          .then(() => {
  ...
  function setupCancelButton() {
    ...
    newCancelBtn.addEventListener("click", function(event) {
      ...
      // Show cancellation in the progress bar
      const progressContainer = document.getElementById('progress-container');
      const progressBar = document.getElementById('progress-bar');
      const progressText = document.getElementById('progress-text');
  ...
  // Setup the cancel button after initialization
  setupCancelButton();

  // Initial visualization
  handleUpdate();

  // Trigger update automatically when the page loads and ensure cancel button is ready
