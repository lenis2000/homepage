...
function addBoundaryRingEdges(Tvertices, edges, n) {
  // 1) Gather indices of boundary vertices
  ...
  // 2) Sort them by angle so we know the circular order
  //    (This uses .re, .im as x,y coordinates in the plane.)
  boundaryIndices.sort((iA, iB) => {
    const vA = Tvertices[iA], vB = Tvertices[iB];
    const aA = Math.atan2(vA.im, vA.re);
    const aB = Math.atan2(vB.im, vB.re);
    return aA - aB;
  });

  // 3) Link each boundary vertex to the next in that circular order
  for (let i = 0; i < boundaryIndices.length; i++) {
    const iA = boundaryIndices[i];
    // wrap around for the “next” index
    const iB = boundaryIndices[(i + 1) % boundaryIndices.length];

    // keep (smaller, bigger) to avoid duplicates in your edges array
    edges.push([
      Math.min(iA, iB),
      Math.max(iA, iB)
    ]);
  ...
  // Initial visualization based on input value
