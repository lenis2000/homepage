...
Module.onRuntimeInitialized = async function() {
  ...
  async function updateVisualization(n, progressInterval) {
    return new Promise(async (resolve, reject) => {
      ...
      // Get abort signal
      const signal = simulationAbortController ? simulationAbortController.signal : null;

      let data;
      try {
        // Check if already aborted before starting computation
        ...
        // Check if aborted during computation
        if (signal && signal.aborted) {
          resolve();
          return;
        }
      ...
      // Check for abort after data processing
      if (signal && signal.aborted) {
        resolve();
        return;
      }
  ...
  function startSimulation() {
    ...
    return simulationAbortController.signal;
  }
  ...
  async function handleUpdate() {
    ...
    // Start simulation and get the abort signal
    startSimulation();
    ...
    try {
      // Run the visualization with the abort signal
      await updateVisualization(nVal, simulationInterval);
