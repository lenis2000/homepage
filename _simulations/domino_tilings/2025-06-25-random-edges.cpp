/*
emcc 2025-06-25-random-edges.cpp -o 2025-06-25-random-edges.js \
 -s WASM=1 \
 -s ASYNCIFY=1 \
 -s "EXPORTED_FUNCTIONS=['_simulateAztec','_simulateAztecWithRegime','_freeString','_getProgress']" \
 ...
 -s SINGLE_FILE=1 \
 -O3 -ffast-math
  mv 2025-06-25-random-edges.js ../../js/

Features:
- Random domino tilings of Aztec diamond with random one-periodic edge weights
- Implements "regime 5.5": Bernoulli edge weights (0.2 or 5.0 with equal probability)
...
char* simulateAztecWithRegime(int n, int regime, double param1, double param2, double param3, double param4) {
    try {
        progressCounter = 0; // Reset progress.

        // Create weight matrix A1a: dimensions 2*n x 2*n with 1-periodic random weights.
        int dim = 2 * n;
        MatrixDouble A1a(dim, dim);
        
        // Generate 1-periodic random weights based on regime
        std::uniform_real_distribution<> dis(0.0, 1.0);
        std::vector<double> random_variables(n);
        ...
        for (int k = 0; k < n; k++) {
            ...
            switch (regime) {
                case 1: {
                    // Regime 1: param1 + 2/sqrt(N) with prob param3, param2 - 1/sqrt(N) with prob param4
                    // Default: param1=1, param2=1, param3=0.5, param4=0.5
                    if (rnd < param3) {
                        random_variables[k] = param1 + 2.0 / sqrtN;
                    } else {
                        random_variables[k] = param2 - 1.0 / sqrtN;
                    }
                    break;
                }
                case 2: {
                    // Regime 2: param1 with prob param3, param2 with prob param4
                    // Default: param1=2, param2=1, param3=1/sqrt(N), param4=(sqrt(N)-1)/sqrt(N)
                    double prob1 = param3; // Should be 1/sqrt(N)
                    if (rnd < prob1) {
                ...
                case 3: {
                    // Regime 3: param1 with prob param3, param2 with prob param4
                    // Default: param1=2, param2=0.5, param3=0.5, param4=0.5
                    if (rnd < param3) {
                        random_variables[k] = param1;
                    } else {
                        random_variables[k] = param2;
                ...
                default: {
                    // Default regime (original regime 5.5): 0.2 or 5.0 with equal probability
                    if (rnd < 0.5) {
                        random_variables[k] = 0.2;
                    } else {
                        random_variables[k] = 5.0;
                    }
                    break;
                }
            }
...
char* simulateAztec(int n) {
    // Backward compatibility wrapper: use default regime (original behavior)
    return simulateAztecWithRegime(n, 0, 0, 0, 0, 0); // regime 0 = default/original
}
