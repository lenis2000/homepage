...
Module.onRuntimeInitialized = async function() {
  ...
  async function handleUpdate() {
    ...
    // Filter out any vertices at (0,0)
    const Tvertices = data.T.filter(v => !(Math.abs(v.re) < 1e-10 && Math.abs(v.im) < 1e-10));
    ...
    sphereGroup = new THREE.Group();
    {
      ...
      Tvertices.forEach(v => {
        ...
        if (Math.abs(v.re) < 1e-10 && Math.abs(v.im) < 1e-10) return;

        const z = OImMap.has(`${v.k},${v.j}`) ? OImMap.get(`${v.k},${v.j}`) : 0;
        const mesh = new THREE.Mesh(sphereGeom, sphereMat);
        mesh.position.set(v.re, -v1.im, z);
