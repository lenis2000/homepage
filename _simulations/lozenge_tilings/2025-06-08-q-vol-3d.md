...
function generateAndDownloadSTLLayer() {
  ...
  // Choose integer shell thickness 'a' in voxels; keep your heuristic but ensure >=1
  const a = Math.max(1, Math.round(Math.min(5, params.N / 10)));
  const S = params.S, T = params.T;

  // 1) Build top surface height map h(x,y)
  const topHM = getHeightMap(paths, params);   // uses your existing v5 getHeightMap

  // 2) Build bottom surface via diagonal erosion by 'a'
  const botHM = erodeHeightMapDiagonal(topHM, a, S, T);

  // 3) Fill the shell: z in [g(x,y), h(x,y))
  const cubeSet = new Set();
  const Ymax = T - S;
  for (let x = 0; x <= S; x++) {
    for (let y = 0; y <= Ymax; y++) {
      const top = HM_get(topHM, x, y);
      const bot = HM_get(botHM, x, y);
      for (let z = bot; z < top; z++) {
        cubeSet.add(`${y},${x},${z}`); // keep the same orientation used elsewhere
      }
    }
  }

  // 4) Emit a closed manifold by emitting only exterior faces
  const stlString = generateSTLFromCubeSet(cubeSet, "lozenge_tiling_layer");
