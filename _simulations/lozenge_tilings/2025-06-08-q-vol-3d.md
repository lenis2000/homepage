...
`;
    ...
    function generatePDF(paths, params, colors, borderWidth) {
        ...
        // Calculate scale to fit the hexagon on the page
        const scaleX = (pdfWidth - 2 * margin) / hexWidth;
        const scaleY = (pdfHeight - 2 * margin) / hexHeight;
        const scale = Math.min(scaleX, scaleY) * 0.8; // Use 80% of max scale for padding

        const pdf = new jsPDF({
            orientation: hexWidth > hexHeight ? 'landscape' : 'portrait',
            unit: 'mm',
            format: 'a4'
        });
        ...
        const offsetY = (pageHeight - hexHeight * scale) / 2;
        ...
        // Helper function to draw a rhombus
        const drawRhombus = (x1, y1, x2, y2, x3, y3, x4, y4, fillColor) => {
            ...
            pdf.setLineWidth(borderWidth * scale * 0.1);
            
            // Transform coordinates to PDF space
            const tx1 = offsetX + (x1 - minX) * scale;
            const ty1 = offsetY + (maxY - y1) * scale;
            const tx2 = offsetX + (x2 - minX) * scale;
            const ty2 = offsetY + (maxY - y2) * scale;
            const tx3 = offsetX + (x3 - minX) * scale;
            const ty3 = offsetY + (maxY - y3) * scale;
            const tx4 = offsetX + (x4 - minX) * scale;
            const ty4 = offsetY + (maxY - y4) * scale;
            
            // Draw filled rhombus using lines() method
            pdf.lines([
                [tx2 - tx1, ty2 - ty1],
                [tx3 - tx2, ty3 - ty2],
                [tx4 - tx3, ty4 - ty3],
                [tx1 - tx4, ty1 - ty4]
            ], tx1, ty1, [1, 1], 'FD'); // Fill and Draw
        };
        ...
        const vertices = [
            ...
            [0, N],
            [S * 0.5 * sqrt3, N + S * 0.5],
            [T * 0.5 * sqrt3, N + (2 * S - T) * 0.5],
            [T * 0.5 * sqrt3, (2 * S - T) * 0.5],
        ...
        // Transform vertices to PDF space and draw hexagon
        const transformedVertices = vertices.map(v => [
            offsetX + (v[0] - minX) * scale,
            offsetY + (maxY - v[1]) * scale
        ]);

        // Draw hexagon outline
        pdf.lines([
            [transformedVertices[1][0] - transformedVertices[0][0], transformedVertices[1][1] - transformedVertices[0][1]],
            [transformedVertices[2][0] - transformedVertices[1][0], transformedVertices[2][1] - transformedVertices[1][1]],
            [transformedVertices[3][0] - transformedVertices[2][0], transformedVertices[3][1] - transformedVertices[2][1]],
            [transformedVertices[4][0] - transformedVertices[3][0], transformedVertices[4][1] - transformedVertices[3][1]],
            [transformedVertices[5][0] - transformedVertices[4][0], transformedVertices[5][1] - transformedVertices[4][1]],
            [transformedVertices[0][0] - transformedVertices[5][0], transformedVertices[0][1] - transformedVertices[5][1]]
        ], transformedVertices[0][0], transformedVertices[0][1], [1, 1], 'S');

        // Draw the actual tiling rhombi
        for (let i = 0; i < T; i++) {
            for (let j = 0; j < N; j++) {
                const currentHeight = paths[j][i];
                const nextHeight = paths[j][i + 1];
                ...
                drawRhombus(x1, y1, x2, y2, x3, y3, x4, y4, fillColor);
            }
        }
