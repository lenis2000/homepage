...
function generateAndDownloadSTLLayer() {
    ...
    // Create height map for the bottom surface using vector (-a,-a,-a)
    // Bottom surface = f(x-a, y-a) - a
    ...
    topHeightMap.forEach((topHeight, key) => {
        ...
        if (topHeightMap.has(shiftedKey)) {
            // Bottom surface = f(x-a, y-a) - a
            const shiftedSurfaceHeight = topHeightMap.get(shiftedKey);
            const bottomHeight = Math.max(0, shiftedSurfaceHeight - a);
            bottomHeightMap.set(key, bottomHeight);
        } else {
            // If shifted point doesn't exist (near boundary), 
            // create a minimal layer with thickness a
            const bottomHeight = Math.max(0, topHeight - a);
            bottomHeightMap.set(key, bottomHeight);
        }
    });

    // Create the layer cubes - only between the two surface heights
    const layerCubeSet = new Set();
    topHeightMap.forEach((topHeight, key) => {
        const [x, y] = key.split(',').map(Number);
        const bottomHeight = bottomHeightMap.get(key) || 0;
        
        // Add cubes only in the layer between bottom and top surfaces
