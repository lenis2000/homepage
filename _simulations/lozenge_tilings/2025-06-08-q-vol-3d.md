...
Module.onRuntimeInitialized = async function() {
    ...
    class Tiling3DVisualizer {
        ...
        // Convert paths to 3D stepped surface representation
        pathsTo3D(paths, N, T, S) {
            ...
            // Create geometry for the stepped surface
            ...
            // Helper to add a square face to the geometry
            const addSquareFace = (corners, color) => {
                // Add 4 vertices for the square
                ...
                // Add two triangles to form the square (with correct winding order)
                indices.push(
            ...
            // First, build the height function from paths
            // paths[i][t] represents particle positions - we need to convert to height function
            const heightFunction = new Map();
            
            // The paths encode non-intersecting lattice paths
            // Height at position (x,y) is determined by counting paths below/above
            for (let x = 0; x <= T; x++) {
                for (let y = 0; y <= N + S; y++) {
                    let height = 0;
                    
                    // Count paths that contribute to height at this position
                    for (let i = 0; i < paths.length; i++) {
                        if (x < paths[i].length) {
                            // If path i at time x is at or above position y, it contributes to height
                            if (paths[i][x] >= y) {
                                height++;
                            }
                        }
                    ...
                    heightFunction.set(`${x},${y}`, height);
                }
            }
            
            // Now create the stepped surface based on the height function
            // Add horizontal faces (top faces of the surface)
            for (let x = 0; x < T; x++) {
                for (let y = 0; y < N + S; y++) {
                    const h = heightFunction.get(`${x},${y}`) || 0;
                    const h_right = heightFunction.get(`${x+1},${y}`) || 0;
                    const h_up = heightFunction.get(`${x},${y+1}`) || 0;
                    const h_diag = heightFunction.get(`${x+1},${y+1}`) || 0;
                    
                    // Check if this is a valid face on the surface
                    // A horizontal face exists if all four corners have the same height
                    if (h === h_right && h === h_up && h === h_diag && h > 0) {
                        // Add horizontal face
                        const corners = [
                            [x, h, y],
                            [x+1, h, y],
                        ...
                        // Determine color based on the lozenge type in 2D projection
                        // This requires checking the paths to determine rhombus type
                        let color = this.colors.gray3; // Default horizontal
                        
                        // Check if this corresponds to a specific rhombus type
                        for (let i = 0; i < paths.length && i < N; i++) {
                            if (x < paths[i].length - 1) {
                                const pathY = paths[i][x];
                                const nextPathY = paths[i][x + 1];
                                
                                // Map path positions to rhombus types
                                if (y === i && pathY === nextPathY) {
                                    color = this.colors.gray1; // Down rhombus
                                } else if (y === i && pathY !== nextPathY) {
                                    color = this.colors.gray2; // Up rhombus
                                }
                            }
                        }
                        
                        addSquareFace(corners, color);
                    }
                    
                    // Add vertical faces where height changes
                    // Face in x-direction
                    if (h !== h_right) {
                        const minH = Math.min(h, h_right);
                        const maxH = Math.max(h, h_right);
                        
                        for (let z = minH; z < maxH; z++) {
                            const corners = [
                                [x + 0.5, z, y],
                                [x + 0.5, z+1, y],
                                [x + 0.5, z+1, y+1],
                            ...
                            addSquareFace(corners, this.colors.gray2);
                        }
                    }
                    
                    // Face in y-direction
                    if (h !== h_up) {
                        const minH = Math.min(h, h_up);
                        const maxH = Math.max(h, h_up);
                        ...
                        for (let z = minH; z < maxH; z++) {
                            ...
                            addSquareFace(corners, this.colors.gray1);
                        }
                    }
                }
            }
            ...
            this.boxGroup.add(surfaceMesh);
            
            // Add wireframe overlay for edges
            const edgesGeometry = new THREE.EdgesGeometry(surfaceGeometry, 1);
