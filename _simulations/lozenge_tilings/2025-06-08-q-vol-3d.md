...
Module.onRuntimeInitialized = async function() {
    ...
    class Tiling3DVisualizer {
        ...
        pathsTo3D(paths, N, T, S) {
            ...
            // DEBUG: Check if paths are already in triplet format
            let pathTriplets = [];
            if (paths.length > 0 && paths[0].length > 0 && Array.isArray(paths[0][0]) && paths[0][0].length === 3) {
                // Paths are already in triplet format (DEBUG mode)
                pathTriplets = paths;
                console.log("DEBUG: Using hardcoded triplet paths", pathTriplets);
            } else {
        ...
        placeAllHorizontalLozenges(pathTriplets, addSquareFace) {
            ...
            for (let pathIdx = 0; pathIdx < pathTriplets.length - 1; pathIdx++) {
                ...
                const horizontalMatrix = this.calculateHorizontalLozengeMatrix(upperPath, lowerPath);
                
                // Place squares where matrix value is 1
                for (let a = 0; a < horizontalMatrix.length; a++) {
                    for (let b = 0; b < horizontalMatrix[a].length; b++) {
                        if (horizontalMatrix[a][b] === 1) {
                            const square = [
        ...
        calculateHorizontalLozengeMatrix(upperPath, lowerPath) {
            // Implement the Mathematica logic: q[z] function and matrix calculation
            // Matrix dimensions: 7 rows (a=0 to 6) × 5 columns (b=0 to 4)
    ...
    async function init() {
        try {
            ...
            // DEBUG MODE: Use hardcoded example paths
            const debugPaths = [
            ...
            // Set up WASM interface with dummy parameters but use debug paths
            ...
            // Update visualization with debug data
            updateVisualization();
