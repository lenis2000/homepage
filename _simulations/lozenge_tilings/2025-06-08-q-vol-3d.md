...
`;
    ...
    function generatePDF(paths, params, colors, borderWidth) {
        ...
        // Add title
        pdf.setFontSize(12);
        pdf.text(`Lozenge Tiling - N=${N}, T=${T}, S=${S}, q=${params.q}`, pageWidth / 2, 15, { align: 'center' });
        ...
        const drawRhombus = (x1, y1, x2, y2, x3, y3, x4, y4, fillColor) => {
            ...
            pdf.polygon(path, 'FD'); // Fill and Draw
        };

        // Draw background hexagon filled with horizontal rhombi
        for (let timeIdx = -1; timeIdx <= T; timeIdx++) {
            for (let height = -(T - S + 2); height <= N + S + 2; height++) {
                const x1 = timeIdx * 0.5 * sqrt3;
                const y1 = height - timeIdx * 0.5;
