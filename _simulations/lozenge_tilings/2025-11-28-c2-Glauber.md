...
Module.onRuntimeInitialized = function() {
    ...
    class LozengeRenderer {
        ...
        drawSideLabels(ctx, sim, centerX, centerY, scale) {
            ...
            centroidX /= sim.boundary.length;
            centroidY /= sim.boundary.length;
            ...
            for (let labelIdx = 0; labelIdx < labels.length; labelIdx++) {
                ...
                // Choose the normal pointing AWAY from centroid (outside)
                const toMidX = midX - centroidX;
                const toMidY = midY - centroidY;
                ...
                // For the notch sides (indices 4,5,6,7: d,e,e,d), flip to be outside the notch
                if (labelIdx >= 4 && labelIdx <= 7) {
                    nx = -nx;
                    ny = -ny;
                }
