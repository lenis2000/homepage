---
title: Glauber dynamics for lozenge tilings of a cutout region
model: lozenge-tilings
author: 'Leonid Petrov'
code:
  - link: 'https://github.com/lenis2000/homepage/blob/master/_simulations/lozenge_tilings/2025-11-26-cutout-region-glauber.md'
...
Module.onRuntimeInitialized = async function() {
    ...
    class GlauberWASM {
        ...
        async initialize() {
            ...
            this.ready = true;
            console.log('Glauber WASM module loaded successfully');
        }

        async init(gridSize, cutX1, cutX2, cutY1, cutY2, holeHeight, maxHeightRatio, bias, mode) {
            if (!this.ready) throw new Error('WASM not ready');

            const ptr = this.initDomain(gridSize, cutX1, cutX2, cutY1, cutY2, holeHeight, maxHeightRatio, bias, mode);
    ...
    async function initSimulation(mode = 0, resetView = true) {
        ...
        try {
            const result = await wasm.init(gridSize, cutX1, cutX2, cutY1, cutY2, holeHeight, maxHeightRatio, bias, mode);
            totalSteps = 0;
            if (resetView) {
                renderer.resetView();
    ...
    console.log('Cutout region Glauber dynamics simulation ready');
};
</script>
