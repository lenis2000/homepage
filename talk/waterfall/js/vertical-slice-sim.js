// Vertical Slice slide - 3D tiling + slice simulation, swaps between q=0.985 and q=0.9
(function() {
    ...
    function tryInitWasm() {
        if (typeof Module !== 'undefined' && typeof Module.cwrap === 'function' && Module.calledRun) {
            if (wasmInterface.initialize()) {
                wasmReady = true;
                console.log('vertical-slice WASM ready');
            }
        }
    }
    ...
    function initThreeJS() {
        const canvas = document.getElementById('vs-3d-canvas');
    ...
    function drawSlice(sliceData) {
        const canvas = document.getElementById('vs-slice-canvas');
