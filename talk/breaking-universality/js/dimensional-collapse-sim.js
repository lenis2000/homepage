/**
 * Dimensional Collapse Slide - Step-by-step 3D waterfall growth
 * Shows q-Racah tiling growing from S=0 to S=80
 */

(function initDimCollapseSim() {
    if (!window.slideEngine) {
        setTimeout(initDimCollapseSim, 50);
        return;
    }
    ...
    function tryInitWasm() {
        if (typeof Module !== 'undefined' && typeof Module.cwrap === 'function' && Module.calledRun) {
            if (wasmInterface.initialize()) {
                wasmReady = true;
                console.log('dimensional-collapse WASM ready');
            }
        }
    }
    ...
    async function initAndAnimate() {
        ...
        async function doStep() {
            if (!isAnimating || wasmInterface.S_param >= S_target) {
                isAnimating = false;
                setCameraPosition(1);  // Ensure final camera position
                return;
            }
