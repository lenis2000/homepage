 ...
 * GFF ≈ (h₁ - h₂) / √2
 * Uses hex (regular hexagon) shape, WebGPU CFTP with CPU fallback
 */

function initLozengeGFFSim() {
    (async function() {
        'use strict';

        const canvas = document.getElementById('lozenge-gff-canvas');
        if (!canvas) return;

        if (typeof LozengeModule === 'undefined') {
            console.error('[LozGFF] LozengeModule not loaded');
            return;
        }
        ...
        async function initGPU() {
            ...
            if (typeof WebGPULozengeEngine === 'undefined') {
                console.log('[LozGFF] WebGPULozengeEngine not available');
                return false;
            }
            try {
                ...
                gpuAvailable = true;
                console.log('[LozGFF] WebGPU engine initialized');
                return true;
            } catch (e) {
                console.log('[LozGFF] WebGPU not available:', e.message);
                gpuAvailable = false;
                return false;
            }
        }
        ...
        try {
            const resp = await fetch('/letters/hex2.json');
            ...
            console.error('[LozGFF] Failed to load hex.json:', e);
            return;
        }
        ...
        function animateCamera(targetPos, targetLookAt, targetZoom, duration, targetUp) {
            ...
            // Disable damping during animation to prevent oscillation
        ...
        async function sampleOneCFTP() {
            ...
            if (useGPU && gpuEngine) {
                try {
                    ...
                    if (minGridData && maxGridData) {
                        ...
                        if (gpuCftpOk) {
                            ...
                            if (coalesced) {
                                ...
                                console.log('[LozGFF] GPU CFTP completed, T=' + T + ', dimers=' + dimers.length);
                                return dimers;
                            }
                            gpuEngine.destroyCFTP();
                        }
                    ...
                    console.log('[LozGFF] GPU CFTP failed, falling back to WASM:', e.message);
                }
            }

            // CPU fallback
            console.log('[LozGFF] Using WASM CFTP');
        ...
        async function doSample() {
            ...
            console.log('[LozGFF] Sample 1: ' + ((t1 - t0) / 1000).toFixed(1) + 's');
            ...
            console.log('[LozGFF] Sample 2: ' + ((t2 - t1) / 1000).toFixed(1) + 's');
            console.log('[LozGFF] Total: ' + ((t2 - t0) / 1000).toFixed(1) + 's');
        ...
        function waitForSlideEngine() {
            if (window.slideEngine) {
                window.slideEngine.registerSimulation('lozenge-gff', {
                    steps: 2,
...
if (typeof LozengeModule !== 'undefined') {
    ...
    window.addEventListener('wasm-loaded', initLozengeGFFSim, { once: true });
}
