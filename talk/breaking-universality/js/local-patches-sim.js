/**
 * Local Patches Slide 3D Tiling (Lazy WebGL loading)
 * Copied from visual/energy — flat (sloped) tilings as universal local limits
 */

function initLocalPatchesSim() {
    (async function() {
        'use strict';

        const canvas = document.getElementById('lp-tiling-canvas');
        if (!canvas) return;
        ...
        const btn = document.getElementById('lp-sample');
        const inputA = document.getElementById('lp-gradient-a');
        const inputB = document.getElementById('lp-gradient-b');
            ...
            try {
                ...
                if (!objMeshCached) {
                    ...
                    objMeshCached = await new Promise((resolve, reject) => {
                        loader.load('/talk/waterfall/images/big_shape.obj', resolve, undefined, reject);
                    });
                    if (!meshGroup) return;
                }
                ...
                console.error('[local-patches] Failed to load big_shape.obj:', e);
            }
        }
        ...
        function waitForSlideEngine() {
            if (typeof window.slideEngine !== 'undefined') {
                ...
                function applyPreset(idx, resetCamera = true) {
                    ...
                    document.getElementById('lp-gradient-a').value = A;
                    document.getElementById('lp-gradient-b').value = B;
...
if (typeof LozengeModule !== 'undefined') {
    ...
    window.addEventListener('wasm-loaded', initLocalPatchesSim, { once: true });
}
