...
\[
  \lim_{n\to\infty} \frac{\alpha_j}{\sqrt{n}}
	\;=\; \sqrt{\frac{\beta}{2}(1-\theta_j\Bigr)}
...
\]
for complex \(z\).  Since $T/\sqrt{n}$ has nonzero entries only on the main and first off-diagonals, one can write down a linear recurrence for the entries
$R_{ij}$
of the resolvent
$R(z)=(T/\sqrt{n}-z I)^{-1}$,
from the equation
...
\begin{equation*}
	...
	\left[f_u''(\theta)+2f(\theta)\right]=\delta(\theta-u).
\end{equation*}
