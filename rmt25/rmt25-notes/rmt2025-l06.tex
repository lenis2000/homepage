...
When \(X=2\) the two solutions in \eqref{eq:critical-points} merge into a double critical point. In this case the equation
\[
z+\frac{1}{z}-2=0
\]
has the unique solution \(z_{cr}=1\). One readily checks that
\[
S(z)=\frac{z^2}{2}+\log z-2z,\qquad S'(z)=z+\frac{1}{z}-2,
\]
satisfies
\[
S'(1)=0,\qquad S''(1)=1-1=0,\qquad S'''(1)=\frac{2}{1^3}=2.
\]
...
\]
A short computation shows that the additional linear terms in the exponent (arising from the factors \(tx\) and \(sy\) in \eqref{eq:K_n_scaling}) contribute, after the change of variables, a factor
\[
\exp\Bigl\{U\,\xi-V\,\eta\Bigr\}.
\]
Taking into account the Jacobians of the rescalings and the prefactor in \eqref{eq:K_n_scaling}, one deduces that the rescaled kernel
\[
\widetilde{K}_n(\xi,\eta)=\frac{1}{n^{1/6}}K_n\Bigl(2\sqrt{n}+\frac{\xi}{n^{1/6}},\,2\sqrt{n}+\frac{\eta}{n^{1/6}}\Bigr)
\]
admits the asymptotic representation
\[
\widetilde{K}_n(\xi,\eta)
=\frac{1}{(2\pi i)^2}\int_{\Gamma_V}dV\int_{\Gamma_U}dU\,
\frac{\exp\Bigl\{\frac{V^3-U^3}{3}+U\,\xi-V\,\eta\Bigr\}}{V-U}\Bigl(1+O\Bigl(n^{-1/3}\Bigr)\Bigr),
\]
where \(\Gamma_U\) and \(\Gamma_V\) are appropriate contours in the complex plane, chosen so that the integrals are convergent (with \(\Gamma_U\) a steepest descent contour for \(U\) and \(\Gamma_V\) a steepest ascent contour for \(V\)).

The double contour integral above is recognized as a standard representation for the \emph{Airy kernel}
\[
K_{\mathrm{Ai}}(\xi,\eta)=\frac{1}{(2\pi i)^2}\int_{\Gamma_V}dV\int_{\Gamma_U}dU\,
...
\]
with \(\mathrm{Ai}(\cdot)\) denoting the Airy function.

We thus arrive at the following result.

\begin{proposition}[Edge asymptotics]
\label{prop:edge}
For the GUE kernel \(K_n(x,y)\) one has
\[
\lim_{n\to\infty}\frac{1}{n^{1/6}}K_n\Bigl(2\sqrt{n}+\frac{\xi}{n^{1/6}},\,2\sqrt{n}+\frac{\eta}{n^{1/6}}\Bigr)
=K_{\mathrm{Ai}}(\xi,\eta),
\]
for all \(\xi,\eta\in\mathbb{R}\). Consequently, the eigenvalue statistics at the edge of the spectrum converge to the Airy point process.
\end{proposition}

\begin{remark}
A rigorous justification of the above asymptotic analysis requires uniform estimates on the error terms in the expansion and careful control of the contour deformations. We refer the interested reader to standard texts on steepest descent methods for such details.
\end{remark}
