...
\begin{definition}[Schur polynomial]
	...
	\begin{equation}
		...
		s_\lambda(x_1,\ldots,x_n) = \sum_{T \in T(\lambda)} \prod_{i=1}^n x_i^{m_i(T)}
	\end{equation}
	where $T(\lambda)$ is the set of all semistandard Young tableaux of shape $\lambda$ with entries from $\{1,2,\ldots,n\}$, and $m_i(T)$ is the number of occurrences of $i$ in the tableau $T$.
\end{definition}
...
\begin{theorem}
	...
	For a fixed Young diagram $\lambda$ and the random array
\end{theorem}
\begin{proof}
