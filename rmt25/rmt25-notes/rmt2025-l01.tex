...
(cf. \Cref{sub:conv-in-dist-problem}).
...
\begin{enumerate}
    ...
    \item The diagonal entries $\{X_{ii}\}$ are i.i.d. real random variables with mean $0$ and variance $\sigma_d$;
    \item The upper triangular entries $\{X_{ij}: i < j\}$ are i.i.d.
...
where $Y$ is a matrix with i.i.d. Gaussian entries $Y_{ij} \sim \mathcal{N}(0, 1)$.
