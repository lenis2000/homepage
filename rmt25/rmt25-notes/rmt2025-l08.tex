...
\begin{equation*}
	\prod_{1\le i<j\le n-1} (\mu_i - \mu_j) \prod_{i=1}^{n} \prod_{j=1}^{n+1} |\mu_i - \lambda_j|^{\beta/2-1}.
\end{equation*}
...
\begin{split}
	...
	\prod_{i<j}(\mu_i-\mu_j) \prod\prod(\mu_j-\lambda_i)^{\beta/2-1} \prod e^{W(\mu_j)} \frac{\prod(z-\lambda_i)}{\prod(z-\mu_j)} \\
	&\hspace{150pt}\times
	\left(W'(z) + \sum \frac{\beta/2-1}{z-\lambda_i} + \sum \frac{1}{z-\mu_j}\right) d\mu_2.
\end{split}
\end{equation}
...
\begin{multline*}
	...
	\mu_1}\left(\underbrace{\prod_{i<j}(\mu_i-\mu_j)
	\prod\prod(\mu_j-\lambda_i)^{\beta/2-1} \prod e^{W(\mu_j)}
	\frac{\prod(z-\lambda_i)}{\prod(z-\mu_j)}}_{(*)}\right)
	\\
	=
