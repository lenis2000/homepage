...
\label{thm:CD}
Let \(\{p_j(x)\}_{j\ge0}\) be a family of orthogonal polynomials with respect to a weight function \(w(x)\) on an interval \(I\subset\mathbb{R}\). Denote by \(\gamma_j\) the leading coefficient of \(p_j(x)\) (so that for monic polynomials, \(\gamma_j=1\)) and by \(h_j\) their squared norms,
\[
...
To establish \eqref{eq:telescoping}, we use the three-term recurrence relation satisfied by the orthogonal polynomials. In its standard form (with \(p_j(x)\) monic), the recurrence is
\[
x\,p_j(x)=p_{j+1}(x)+\alpha_j\,p_j(x)+\beta_j\,p_{j-1}(x),\qquad j\ge0,
\]
with the convention \(p_{-1}(x)=0\). (One has \(\beta_j=\frac{h_j}{h_{j-1}}\) for monic orthogonal polynomials.)
This recurrence comes from the two facts:
\begin{enumerate}
...
Write the recurrence for both \(p_j(x)\) and \(p_j(y)\):
\[
\begin{aligned}
...
\]
Dividing by \(h_j\) and summing over \(j=0,\ldots,n-1\) yields
\[
(x-y)S_n(x,y)
...
\]
A reindexing of the sums shows that they telescope, leaving only the boundary term at \(j=n-1\). In particular, one finds
\[
(x-y)S_n(x,y)=\frac{1}{h_{n-1}}\Bigl[p_n(x)p_{n-1}(y)-p_{n-1}(x)p_n(y)\Bigr].
\]
This proves \eqref{eq:telescoping}.

Finally, the factor \(1/h_{n-1}\) may be expressed in terms of the leading coefficients by the relation
\[
\frac{1}{h_{n-1}}=\frac{\gamma_{n-1}}{\gamma_n},
\]
which completes the derivation of \eqref{eq:CD}.
