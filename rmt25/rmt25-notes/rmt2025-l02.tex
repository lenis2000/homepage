...
\[
	Y_{n,k}\coloneqq\int_{\mathbb{R}}x^k\,d\nu_n(x)
...
\begin{proposition}[Variance bound]
	...
	we have
	\begin{equation*}
		\operatorname{Var}(Y_{n,k})\le \frac{m_k}{n^2}.
	\end{equation*}
\end{proposition}
...
Since \(\lvert Y_{n,k}\rvert = \bigl|\int x^k\,d\nu_n(x)\bigr|\) stays bounded for each \(k\), one readily checks
...
\begin{equation}
  ...
  \;\le\; \frac{C}{M^k}
\end{equation}
for any positive integer \(k\).  By choosing \(k\) large, we see that \(\nu_n\) puts arbitrarily little mass outside any large interval \([-m,m]\).  Thus, the
...
\[
   ...
   \text{for all $k\ge1$.}
\]
By \emph{Carleman’s criterion} (\Cref{prop:carleman}), the measure \(\nu^*\) is uniquely determined by its moments.  Hence \(\nu^*\) must coincide with \(\mu_{\mathrm{sc}}\).

\smallskip

\noindent
\textbf{Conclusion.}
Any subsequence of \(\{\nu_n\}\) has a further sub-subsequence convergent to \(\nu\).  By a standard diagonal argument, this forces \(\nu_n\to\nu\) in the weak topology.  This completes the proof that the ESD of our Wigner matrix (rescaled by \(\sqrt{n}\)) converges to the semicircle distribution almost surely.
