...
and follows these \href{https://www.samuelfhopkins.com/docs/rsk.pdf}{notes} by
Sam Hopkins.
...
\begin{tikzpicture}[scale=0.9]
				% Draw the shape of the R array with 4 boxes in bottom row, 3 in middle, 2 in top
				% Bottom row (i=3)
				\foreach \j in {1,2,3,4} {
												\node[rectangle,draw,minimum size=0.9cm] (R3\j) at (\j,1) {$R_{3\j}$};
				}
				% Middle row (i=2) - including the box being added
				\foreach \j in {1,2,3} {
												\node[rectangle,draw,minimum size=0.9cm] (R2\j) at (\j,2) {$R_{2\j}$};
				}
				% Top row (i=1)
				\foreach \j in {1,2} {
												\node[rectangle,draw,minimum size=0.9cm] (R1\j) at (\j,3) {$R_{1\j}$};
				}

				% Highlight the current position (2,3) where toggle is applied (the new box in middle row)
				\node[rectangle,draw,fill=yellow!30,minimum size=0.9cm] at (3,2) {$R_{2,3}$};

				% Highlight the diagonal λ (from left)
				\draw[->,thick,blue] (2,2) -- (2.6,2);
				\node[rectangle,draw,fill=blue!20,minimum size=0.9cm] at (2,2) {$R_{2,2}$};
				\node[blue] at (1.3,2) {$\lambda^{(2,3)}$};

				% Highlight the diagonal μ (from above)
				\draw[->,thick,red] (3,3) -- (3,2.4);
				\node[dashed,rectangle,draw,fill=red!20,minimum size=0.9cm] at (3,3) {$R_{1,3}$};
				\node[red] at (3,3.7) {$\mu^{(2,3)}$};

				% Highlight the diagonal κ (from above-left)
				\draw[->,thick,green] (2,3) -- (2.6,2.6);
				\node[rectangle,draw,fill=green!20,minimum size=0.9cm] at (2,3) {$R_{1,2}$};
				\node[green] at (1.3,3) {$\kappa^{(2,3)}$};

				% Show the weight W being applied
				\node[circle,draw,fill=gray!20] at (5,2) {$W_{2,3}$};
				\draw[->,thick] (5,2) -- (3.7,2);
\end{tikzpicture}
\caption{Illustration of the RSK toggle operation. The colored cells represent the three diagonals involved in computing the new value at position $(2,2)$.}
\label{fig:toggle_operation}
\end{figure}
