...
\begin{remark}
	The quantity $Y(n,t)$ appears in many contexts besides the LPP.
	Namely,
...
\[ Z(t) := \big( Y(1,t),\, Y(2,t),\, \dots,\, Y(n,t)\big)\,\in \mathbb{W}^n, \]
where we list the values in increasing order $Y(1,t)\le
Y(2,t)\le \cdots \le Y(n,t)$.\footnote{We have $Y(1,t)\le
\cdots\le Y(n,t)$ almost surely because giving the path more
freedom to move down can only increase the maximum weight.
This is easily checked from \eqref{eq:LPP-recursion}. Thus
$Z(t)\in \mathbb{W}^n$ indeed.} In particular, $Y(n,t)$ is
