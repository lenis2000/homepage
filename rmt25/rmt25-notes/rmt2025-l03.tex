...
This scaling ensures that each \(\alpha_j^2\) has the
appropriate variance required for the GOE ensemble,
specifically matching the variance structure of the original
Wigner matrix. The chi-square distribution's properties,
such as its mean being \(\nu\) and its variance being
\(2\nu\), play a crucial role in maintaining the tridiagonal
matrix's spectral characteristics that lead to the
semicircle law.
