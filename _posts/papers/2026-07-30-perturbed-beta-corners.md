...
coauthors:
  - name: Leonid Petrov
  - name: Jiaming Xu
    web: https://sites.google.com/wisc.edu/jiaming/home
comments: false
...
# journal-year:
show-date: true
pdf: 53-perturbed-beta-corners.pdf
post-pdf: true
pages: 31
cv-number: 53
published: true
...
We extend the construction to all $\beta>0$ via multivariate Bessel functions, and analyze the crystallization ($\beta\to\infty$) of the resulting interlacing array, proving a law of large numbers and a central limit theorem in two regimes. For fixed perturbation, the eigenvalue repulsion dominates, and the array crystallizes on the same lattice of polynomial-derivative roots, with the same discrete Gaussian free field fluctuations, as in the unperturbed case treated by Gorin and Marcus ([arXiv:1706.07393](https://arxiv.org/abs/1706.07393)). New phenomena appear in the second regime, where the $a_i$'s grow linearly in $\beta$. Then the external source competes with the repulsion at leading order. Here the array freezes on a deformed lattice characterized by a coupled system of optimality equations. The fluctuations are governed by the same discrete Gaussian free field, now attached to the deformed lattice.
